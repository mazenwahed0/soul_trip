...
import '../../../../features/category_trips/data/repositories/category_trips_likes_repository.dart';
import 'category_trips_likes_state.dart';

class CategoryTripsLikesCubit extends Cubit<CategoryTripsLikesState> {
  final CategoryTripsLikesRepository _repository;
  final String _userId;
  Map<String, bool> _likedTrips = {};
  StreamSubscription? _likesSubscription;

  CategoryTripsLikesCubit(this._repository, this._userId)
    : super(CategoryTripsLikesInitial()) {
  ...
  void _listenToLikesChanges() {
    ...
    _likesSubscription = _repository
        ...
        .listen(
          (likes) {
            _likedTrips = likes;
            emit(CategoryTripsLikesLoaded(Map<String, bool>.from(_likedTrips)));
          },
          onError: (error) {
            emit(CategoryTripsLikesError('Failed to listen to likes: $error'));
          },
        );
  }
  ...
  Future<void> toggleLike(String tripId) async {
    ...
    try {
      if (currentLikeStatus) {
        ...
        result.fold(
          (failure) {
            // Revert on error
            _likedTrips[tripId] = currentLikeStatus;
            emit(CategoryTripsLikesError(failure.message));
          },
          (_) {
      ...
      // Revert on error
      _likedTrips[tripId] = currentLikeStatus;
      emit(CategoryTripsLikesError('Failed to toggle like'));
    }
  }
